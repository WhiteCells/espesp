...
DEFAULT_INTERRUPT_ECHO_MARGIN_DB = 8.0
DEFAULT_INTERRUPT_MAX_CONFIRM_MS = 640
PLAYBACK_ECHO_FLOOR_DECAY = 0.98
...
class Settings:
    ...
    interrupt_max_confirm_ms: int
    interrupt_echo_margin_db: float
...
class ClientState:
    ...
    def is_discarded_turn(self, turn_id: int | None) -> bool:
        return self.discard_audio and turn_id is not None and turn_id == self.discard_turn_id

    def mark_response_started(self, turn_id: int | None) -> bool:
        if self.is_discarded_turn(turn_id):
            return False
        self.response_active = True
        self.response_turn_id = turn_id
        if turn_id is not None and turn_id != self.discard_turn_id:
...
def parse_args(argv: list[str] | None = None) -> Settings:
    ...
    parser.add_argument("--output-sample-rate", type=int, default=env_int("VCLIENT_OUTPUT_SAMPLE_RATE", 48000))
    ...
    parser.add_argument("--vad-pre-roll-ms", type=int, default=env_int("VCLIENT_VAD_PRE_ROLL_MS", 320))
    ...
    parser.add_argument(
        ...
        type=int,
        default=env_int("VCLIENT_INTERRUPT_MIN_TURN_MS", 192),
        help="speech duration required before cancelling an active response",
    )
    parser.add_argument(
        "--interrupt-max-confirm-ms",
        type=int,
        default=env_int("VCLIENT_INTERRUPT_MAX_CONFIRM_MS", DEFAULT_INTERRUPT_MAX_CONFIRM_MS),
        help="maximum speech duration to buffer while confirming an interrupt is not playback echo",
    )
    parser.add_argument(
        "--interrupt-echo-margin-db",
        type=float,
        default=env_float("VCLIENT_INTERRUPT_ECHO_MARGIN_DB", DEFAULT_INTERRUPT_ECHO_MARGIN_DB),
        help="extra microphone level over playback leakage required before interrupting",
    )
    parser.add_argument(
        "--input-device",
    ...
    if args.interrupt_max_confirm_ms < args.interrupt_min_turn_ms:
        parser.error("--interrupt-max-confirm-ms must be greater than or equal to --interrupt-min-turn-ms")
    if args.interrupt_echo_margin_db < 0:
        parser.error("--interrupt-echo-margin-db must be greater than or equal to 0")
    ...
    return Settings(
        ...
        interrupt_max_confirm_ms=args.interrupt_max_confirm_ms,
        interrupt_echo_margin_db=args.interrupt_echo_margin_db,
    )
...
async def receive_loop(ws: Any, player: RawAudioPlayer, state: ClientState) -> None:
    async for message in ws:
        ...
        if message_type == "ready":
            ...
            if state.mark_response_started(turn_id):
                print("[assistant] ", end="", flush=True)
        elif message_type == "llm_delta":
            if not state.is_discarded_turn(turn_id):
...
async def vad_record_loop(ws: Any, settings: Settings, player: RawAudioPlayer, state: ClientState) -> None:
    ...
    async def finish_segment(reason: str) -> None:
        ...
        if audio_started:
            ...
            print(f"[interrupt] {reason}，忽略", flush=True)
        speech_active = False
        audio_started = False
        deferred_interrupt = False
        ignored_segment = False
    ...
    def update_playback_echo_floor(response_active: bool) -> None:
        nonlocal playback_echo_floor
        if response_active and not speech_active:
            playback_echo_floor = max(
                vad.last_rms,
                playback_echo_floor * PLAYBACK_ECHO_FLOOR_DECAY,
            )
        elif not response_active:
            playback_echo_floor = 0.0

    def interrupt_level_confirmed() -> bool:
        if playback_echo_floor <= 0:
            return True
        required_rms = playback_echo_floor * (10.0 ** (settings.interrupt_echo_margin_db / 20.0))
        return vad.last_rms >= max(settings.vad_noise_gate_rms, required_rms)
        ...
        with sd.RawInputStream(
            ...
            while True:
                ...
                if not speech_active:
                    ...
                    update_playback_echo_floor(response_active)
                    ...
                    if ignored_segment:
                        ...
                        print(
                            "[interrupt] 检测到插话，确认中... "
                            f"(mic={dbfs(vad.last_rms):.1f} dBFS, "
                            f"floor={dbfs(playback_echo_floor):.1f} dBFS)",
                            flush=True,
                        )
                    else:
                ...
                if not ignored_segment:
                    if audio_started:
                        ...
                        if (
                            deferred_interrupt
                            and speech_ms >= settings.interrupt_min_turn_ms
                            and interrupt_level_confirmed()
                        ):
                            await open_audio_turn(interrupted=True)
                        elif (
                            deferred_interrupt
                            and speech_ms >= settings.interrupt_max_confirm_ms
                        ):
                            await finish_segment("播放漏音未通过插话音量确认")
