...
- `sda_io_num`：SDA GPIO，来自 `CONFIG_ESPESP_I2C_SDA_GPIO`。
- `scl_io_num`：SCL GPIO，来自 `CONFIG_ESPESP_I2C_SCL_GPIO`。
...
- `xfer_timeout_ms`：本次探测超时，来自 `CONFIG_ESPESP_I2C_PROBE_TIMEOUT_MS`。
...
- `CONFIG_ESPESP_I2C_SDA_GPIO`：SDA 引脚。
- `CONFIG_ESPESP_I2C_SCL_GPIO`：SCL 引脚。
- `CONFIG_ESPESP_I2C_PROBE_TIMEOUT_MS`：探测超时。

## 注意事项

- I2C 需要上拉，内部上拉只适合短线低速场景。
