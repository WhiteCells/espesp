...
- `sda_io_num = CONFIG_CASE2_DISPLAY_SDA_GPIO`：SDA GPIO。
- `scl_io_num = CONFIG_CASE2_DISPLAY_SCL_GPIO`：SCL GPIO。
...
- `device_address = CONFIG_CASE2_DISPLAY_I2C_ADDR`：OLED 地址，常见是 `0x3C` 或 `0x3D`。
- `scl_speed_hz = CONFIG_CASE2_DISPLAY_I2C_HZ`：I2C 时钟频率。
...
- `write_buffer`：发送缓冲区。
- `write_size`：发送字节数。
- `xfer_timeout_ms`：超时，来自 `CONFIG_CASE2_DISPLAY_TIMEOUT_MS`。
...
- `CONFIG_CASE2_DISPLAY_SDA_GPIO`：SDA 引脚。
- `CONFIG_CASE2_DISPLAY_SCL_GPIO`：SCL 引脚。
- `CONFIG_CASE2_DISPLAY_I2C_ADDR`：OLED I2C 地址。
- `CONFIG_CASE2_DISPLAY_I2C_HZ`：I2C 频率。
- `CONFIG_CASE2_DISPLAY_TIMEOUT_MS`：I2C 传输超时。
