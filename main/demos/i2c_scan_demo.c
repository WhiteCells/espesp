...
esp_err_t i2c_scan_demo_run(void)
{
    i2c_master_bus_config_t bus_config = {
        .i2c_port = I2C_NUM_0,
        .sda_io_num = CONFIG_CASE2_I2C_SDA_GPIO,
        .scl_io_num = CONFIG_CASE2_I2C_SCL_GPIO,
    ...
    ESP_LOGI(TAG, "scan I2C bus: SDA=GPIO%d, SCL=GPIO%d, timeout=%d ms",
             CONFIG_CASE2_I2C_SDA_GPIO,
             CONFIG_CASE2_I2C_SCL_GPIO,
             CONFIG_CASE2_I2C_PROBE_TIMEOUT_MS);

    int found = 0;
    for (uint8_t address = 0x03; address < 0x78; address++) {
        esp_err_t ret = i2c_master_probe(bus_handle, address, CONFIG_CASE2_I2C_PROBE_TIMEOUT_MS);
        if (ret == ESP_OK) {
